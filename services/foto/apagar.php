...
#Verificar se os dados vindos do Foto estão preenchidos
if(!empty($data->idfoto)){

    $foto->idfoto=$data->idfoto;

    if($foto->apagarfoto()){
        header("HTTP/1.0 200");
        echo json_encode(array("mensagem"=>"Foto apagado com sucesso!"));
    }
    else{
        header("HTTP/1.0 400");
        echo json_encode(array("mensagem"=>"Não foi possível apagar o Foto"));
    }
}
else{
    header("HTTP/1.0 400");
